/**
 * System prompts for the agentic care companion.
 *
 * The prompt is assembled from four layers so each can be tuned independently:
 *   1. Identity + non-negotiable safety rules (never changes per user)
 *   2. Stage instructions (awareness / treatment / vigilance)
 *   3. Therapeutic technique instructions (MI, CBT, ACT)
 *   4. Live patient context + retrieved memory
 *
 * Prompts live server-side so they can be improved without an app release.
 */

export type Stage = 'awareness' | 'treatment' | 'vigilance';
...
- Never use the words "obese person", "fat", "morbidly obese" about the user. Say "living with obesity".
...
BEHAVIOUR
- Open the conversation by understanding their situation before giving information. One question at a time.
...
- If they ask about eligibility, call the "check_eligibility" tool rather than guessing. Ask for the
  missing measurements conversationally, not as a form.
...
- If a check-in is due, ask for it naturally inside the conversation using the "request_check_in" tool,
...
function formatContext(ctx: PatientContext): string {
  ...
  push('Account', ctx.isAnonymous ? 'anonymous (no account, do not ask for identity)' : 'registered patient');
  ...
  if (ctx.activeMedications?.length) {
    lines.push(
      `- Active medication: ${ctx.activeMedications
        .map((m) => `${m.name} ${m.strength} ${m.frequency}${m.nextDoseAt ? ` (next dose ${m.nextDoseAt})` : ''}`)
        .join('; ')}`,
    );
  }
  ...
  if (ctx.conversationSummary) {
    lines.push('', `EARLIER IN THIS CONVERSATION: ${ctx.conversationSummary}`);
  }
...
/** Prompt used to compress an old conversation into durable memory. */
export const MEMORY_EXTRACTION_PROMPT = `
You are maintaining the long-term memory of an obesity care companion.

From the conversation below, extract only durable facts that would change how you support this person in
three months. Ignore small talk, transient symptoms already logged elsewhere, and anything the app
already stores structurally (weight, doses, appointments).

Return strict JSON:
{
  "summary": "<=60 words, third person, factual",
  "memories": [
    { "kind": "preference|concern|goal|fact|barrier", "content": "<=25 words", "salience": 0.0-1.0 }
  ]
}

Rules:
- At most 6 memories.
- No medical advice, no diagnoses, no speculation.
- Do not record identifying details the person did not volunteer.
- If nothing durable was said, return an empty memories array.
`.trim();

/** Prompt used to turn clinical notes into patient-friendly language. */
export const DOCTOR_NOTE_PROMPT = `
You translate a doctor's clinical note into language the patient can act on.

Rules:
- Explain what it means for the patient, in second person ("You'll continue...").
- Preserve every instruction exactly. Do not change a dose, a drug, a frequency or a duration.
- Do not add advice the doctor did not give. Do not soften a warning.
- Explain jargon in brackets the first time it appears.
- 3 to 6 short sentences. Warm, plain, non-alarming.
- End with what to watch for and when to contact the doctor, only if the note implies it.

Return strict JSON: { "patient_friendly": "...", "watch_for": ["..."], "contact_doctor_if": ["..."] }
`.trim();

/** Prompt used to structure a prescription image/text into medication rows. */
export const PRESCRIPTION_EXTRACTION_PROMPT = `
You extract structured medication data from an Indian prescription.
  ...
  "medications": [
    {
      ...
      "form": "injection"|"tablet"|"capsule"|"syrup"|"other",
      "strength": string,
      "dose_amount": number,
      "dose_unit": string,
      "frequency": "weekly"|"daily"|"twice_daily"|"thrice_daily"|"as_needed",
      "times_of_day": ["HH:MM"],
...
Rules:
- Transcribe only what is written. Never infer a medicine that is not on the page.
- Indian shorthand: OD = once daily, BD = twice daily, TDS = three times daily, HS = at night,
  SOS/PRN = as needed, 1-0-1 = morning and night, 1-1-1 = three times a day.
- Weekly injections default to times_of_day ["09:00"] and days_of_week [the weekday written, else null].
- Default oral timings: OD 09:00; BD 09:00 and 21:00; TDS 09:00, 14:00, 21:00; HS 22:00.
- GLP-1 and insulin pens get storage_note "Keep refrigerated at 2-8 °C. Do not freeze."
- If the image is unreadable, return an empty medications array with confidence 0.
`.trim();

export { LANGUAGE_INSTRUCTION, STAGE_PROMPTS, IDENTITY };
